...
MyoData * getMyoData(Myo * myo, bool createIfNotExist = true)
{
	...
	NSLog(@"Myo Not found by handle : %s\n",getMyoID(myo).c_str());
	return NULL;
}
...
MyoData * getMyoByMacAddress(const char * macAddress)
{
	...
	//NSLog(@"Myo Not found by mac address : %s\n",macAddress);
	return NULL;
}
...
public:
	...
	{
		//NSLog(@"Accel data ! %f %f %f",accel[0],accel[1],accel[2]);
		MyoData * mData = getMyoData(myo);
	...
	void onGyroscopeData (Myo *myo, uint64_t timestamp, const Vector3< float > &gyro)
	{
		...
		//NSLog(@"Gyro data ! %f %f %f",gyro[0],gyro[1],gyro[2]);
		MyoData * mData = getMyoData(myo);
...
{
	NSLog(@"myo run thread creation and init !\n");
	...
	{
		NSLog(@"Error creating Hub or Listener : %s\n",e.what());
	...
	NSLog(@"Hub is ready, Launch Hub run loop\n");
	while(!exitRunThread)
	{
		try
		{
...
extern "C"
{
	...
	FREObject init(FREContext ctx, void* funcData, uint32_t argc, FREObject argv[])
	{
		...
		try
		{
			...
			if (pThreadResult){
				NSLog(@"MyoExtension :: Error on Myo run thread creation\n");
			}
			
		}catch(const std::exception& e)
		{
			NSLog(@"MyoExtension :: Error on Myo Init (%s)\n",e.what());
	...
	void MyoContextInitializer(void* extData, const uint8_t* ctxType, FREContext ctx, uint32_t* numFunctionsToSet,  const FRENamedFunction** functionsToSet) {
		...
		NSLog(@"MyoExtension v0.8.1 by Ben kuper, based on MyoSDK v0.8.1\n");
